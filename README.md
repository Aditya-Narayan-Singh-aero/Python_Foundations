# Python Foundations

Welcome to my repository for core Python development! This repository tracks my journey as I build a rock-solid foundation in programming fundamentals before transitioning into complex engineering applications.

---

## 🚀 Projects Included

* **The Whispering Path**
  * **Description:** A text-based interactive adventure game built using Python logic, conditional branching, and user input handling.
  * **Concepts Used:** Control flow (if/elif/else), functions, loops, and terminal-based user interaction.
  * **File:** `The Whispering Path.ipynb`

* **Aircraft Performance Analysis**
  * **Description:** A flight dynamics tool that calculates essential performance metrics, including range, endurance, center of gravity, and kinematic motion for an aircraft.
  * **Concepts Used:** Modular function-based structure, physical modeling, and kinematic equations.
  * **File:** `aircraft_performance_analysis.py`

* **CLI Countdown Timer**
  * **Description:** A robust, interactive countdown timer that converts total seconds into a formatted `HH::MM::SS` clock layout with zero-padding.
  * **Concepts Used:** Time manipulation modules, arithmetic operators (modulo and integer division), conditional loops, and string formatting (f-strings).
  * **File:** `python-countdown-timer.ipynb`

* **Interactive Shopping Cart**
  * **Description:** A terminal-based shopping cart system that tracks items, unit prices, and quantities to calculate total batch costs and display a formatted receipt.
  * **Concepts Used:** While loops, user input validation, lists, parallel iteration via `zip()`, and string formatting.
  * **File:** `shopping_cart.ipynb`

* **Python Quiz Game**
  * **Description:** An interactive terminal-based trivia game that tests users on science fundamentals, handles user input validation with infinite loops, tracks scores, and displays a comprehensive summary of correct answers versus user guesses.
  * **Concepts Used:** Tuples and lists for data structuring, `for` and `while` loops, input validation, conditional logic, and parallel sequence tracking.
  * **File:** `quiz_game.ipynb`

---

## 🛠️ Skills Covered

* Core Python Syntax & Data Types
* Logic, Conditionals, and Control Flow
* Functional Programming Fundamentals
* Physics-based Computational Logic
* Time Management & String Formatting
* Data Collection & Iteration (Lists, Zip)
* Version Control via Git & GitHub

---

## 📈 Future Goals

Once I master the core programming foundations through these initial projects, this repository will serve as the launchpad for my transition into advanced scientific computing, physics simulations, and aerospace/mechanical engineering applications.
